...
fn run() -> Result<(), pa::Error> {
    ...
    let input_params = pa::StreamParameters::<u8>::new(def_input, CHANNELS, INTERLEAVED, latency);
    ...
    let output_params = pa::StreamParameters::<u8>::new(def_output, CHANNELS, INTERLEAVED, latency);
    ...
    let mut buffer: VecDeque<u8> = VecDeque::with_capacity(FRAMES as usize * CHANNELS as usize);
    ...
    'stream: loop {
        ...
        if out_frames > 0 && buffer_frames > 0 {
            ...
            println!("n_write_samples {}", n_write_samples);
