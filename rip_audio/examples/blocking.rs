...
fn run() -> Result<(), pa::Error> {
    ...
    let i_settings = pa::InputStreamSettings::new(input_params, SAMPLE_RATE, FRAMES);
    let o_settings = pa::OutputStreamSettings::new(output_params, SAMPLE_RATE, FRAMES);

    let mut i_stream = pa.open_blocking_stream(i_settings)?;
    let mut o_stream = pa.open_blocking_stream(o_settings)?;
    ...
    'stream: loop {
        ...
        if in_frames > 0 {
            let input_samples = i_stream.read(in_frames)?;
        ...
        let out_frames = if out_frames >= in_frames { in_frames } else { in_frames };
